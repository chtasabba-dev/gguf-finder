...
import { ArrowDownToLine, ArrowRight, Copy, ExternalLink, FileCode2, FolderSearch, Heart, Loader2, Search, Terminal, X } from "lucide-react";
...
export default function Home() {
  ...
  const visibleProfileRepos = useMemo(() => profileRepos.filter((repo) => filterMode === "all" || getPcSuitability(detectParameterSize(repo.id), pcProfile.ramGb, language).status === filterMode), [profileRepos, filterMode, pcProfile.ramGb, language]);
  const visibleFiles = useMemo(() => {
    const filtered = !suitability || filterMode === "all" || suitability.status === filterMode ? [...files] : [];
  ...
  }, [files, filterMode, suitability, sortMode]);
