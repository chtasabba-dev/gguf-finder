...
import { buildFileUrl, buildRepoUrl, findProfileRepositories, findRepository, getFriendlyError, HuggingFaceError, validateOwner, validateRepoId, type ProfileRepository, type RepositoryResult } from "@/lib/huggingface";
...
// Dark console UI: compact semantic model rows prioritize fast, high-contrast scanning.
...
export default function Home() {
  ...
  const [loadingProfileRepoId, setLoadingProfileRepoId] = useState<string | null>(null);
  ...
  const loadRepository = async (repoId: string, returnToProfile = false) => {
    if (returnToProfile) setLoadingProfileRepoId(repoId); else setState("loading");
    setError(""); setRestrictedRepoId(null); setCanReturnToProfile(returnToProfile);
    ...
    catch (caught) { const restricted = caught instanceof HuggingFaceError && caught.kind === "auth"; setRestrictedRepoId(restricted ? repoId : null); setError(restricted ? t.restrictedAccess : getFriendlyError(caught)); setState("error"); }
    finally { setLoadingProfileRepoId(null); }
  };
  const search = async (event?: FormEvent) => {
    ...
    setState("loading"); setError(""); setRestrictedRepoId(null); setCanReturnToProfile(false);
  ...
  return <div className="dark-app" dir={t.dir}>
    ...
    <main className="dark-main">
      ...
      {state === "error" && <div className="dark-message dark-error"><X size={28} /><p>{error}</p>{restrictedRepoId ? <a className="dark-request-access" href={buildRepoUrl(restrictedRepoId)} target="_blank" rel="noreferrer">{t.requestAccess} <ExternalLink size={15} /></a> : <button onClick={() => void search()}>{t.retry}</button>}</div>}
          ...
          {visibleProfileRepos.length === 0 ? (
            ...
            <ul className="dark-profile-list" aria-label={`${owner} GGUF models`}>
              {visibleProfileRepos.map((repo) => {
                ...
                return (
                  <li className={`dark-profile-row${loadingProfileRepoId === repo.id ? " is-loading" : ""}`} key={repo.id}>
                    <div className="dark-profile-row-info">
                      <div className="dark-card-badges"><span className="dark-genre-badge">{taskLabels[genre]}</span>{repo.gated ? <span className="dark-restricted-badge">{t.restricted}</span> : null}<span className={`dark-card-suitability dark-card-suitability--${rowSuitability.status}`}>{rowSuitability.label}</span></div>
                    ...
                    <div className="dark-profile-row-actions">
                      ...
                      <button className="dark-open-button dark-profile-view-button" type="button" disabled={Boolean(loadingProfileRepoId)} onClick={() => void loadRepository(repo.id, true)}>{loadingProfileRepoId === repo.id ? <><Loader2 size={15} className="spin" /> {t.loadingDetails}</> : <>{t.viewFiles} <ArrowDownToLine size={15} /></>}</button>
                    </div>
